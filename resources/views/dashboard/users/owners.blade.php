...
@section('header')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
        <div class="d-block mb-4 mb-md-0">
            ...
            <h2 class="h4">Liste des propriétaires</h2>
        </div>
    ...
    <div class="card card-body border-0 shadow table-wrapper table-responsive">
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <tr>
                    <td class="serial">{{$owner->id}}.</td>

                    <td> {{$owner->first_name}} </td>
                    <td>  <span class="name">{{$owner->last_name}}</span> </td>
                    <td> <span class="product">{{$owner->email}}</span> </td>
                    <td><span class="">{{$owner->phone_number}}</span></td>

                    <td>{{$owner->birthdate}}</td>
                    <td>{{$owner->sex}}</td>
                    <td>
                        <form action="{{route('owners.change.customers',$owner->id)}}" method="POST">
                            @csrf
                            <button type="submit">Rendre client</button>
                        </form>
        ...
        {{ $owners->links('backend.layouts.paginate') }}
